...
export default {
  ...
  methods: {
    ...
    syncConnections () {
        var that = this
        return axios.post(that.config.getAddress("LIST_CONNECTIONS"),{token:Cookies.get('token')})
                    .then(response => {
    ...
    removeConnection(connection) {
        var that = this
        axios.post(that.config.getAddress("DELETE_CONNECTION"), {uuid: connection.uuid,token:Cookies.get('token')})
             .then(response => {
                 that.initData()
             })
             .catch(error => {
