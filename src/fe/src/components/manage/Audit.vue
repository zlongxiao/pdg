...
export default {
  ...
  methods: {
    ...
    syncAuditRecords () {
        var that = this
        return axios.post(that.config.getAddress("LIST_AUDIT_RECORDS"), {current_page: that.currentPage, records_per_page: that.recordsPerPage,token:Cookies.get('token')})
                    .then(response => {
