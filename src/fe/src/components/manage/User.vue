...
export default {
  ...
  methods: {
    ...
    syncUsers () {
        var that = this
        return axios.post(that.config.getAddress("LIST_USERS"),{token:Cookies.get('token')})
                    .then(response => {
    ...
    removeUser(user) {
        var that = this
        axios.post(that.config.getAddress("REMOVE_USER"), {username: user.username,token:Cookies.get('token')})
             .then(response => {
                 that.initData()
             })
             .catch(error => {
    ...
    createUser () {
        var that = this
        axios.post(that.config.getAddress("CREATE_USER"), {token:Cookies.get('token'),username: that.createUserForm.username, password: that.createUserForm.password, role: that.createUserForm.role, mobile: that.createUserForm.mobile, mail: that.createUserForm.mail, wx: that.createUserForm.wx})
             .then(response => {
                 that.initData()
             })
             .catch(error => {
